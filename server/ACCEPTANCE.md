...
- [ ] 200 rapid connection attempts throttled; existing members unaffected — per-IP cap (HTTP 429 on the 5th concurrent connection, XFF-aware) is **automated: pass** (`S3: per-IP connection cap…`). fail2ban jail on the vhost log is **manual: to verify** (RUNBOOK S3.4); regex checked offline against sample `combined` lines. Caveat: a fail2ban ban also cuts legitimate clients behind the *same* IP.
